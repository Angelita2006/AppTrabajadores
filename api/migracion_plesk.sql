...
BEGIN
    ...
    ELSIF (TG_OP = 'UPDATE') THEN
        -- Si estamos en la tabla fichajes, permitimos la actualización EXCLUSIVAMENTE si 
        -- el único campo que cambia es el estado (el resto de la fila permanece idéntica).
        IF TG_TABLE_NAME = 'fichajes' THEN
            -- Comparamos si el estado cambió, pero permitimos que pase 
            -- sin importar cómo se llamen las columnas de tiempo.
            IF (NEW.estado IS DISTINCT FROM OLD.estado) THEN
                RETURN NEW;
            END IF;
        END IF;

        -- Si intentan modificar otra tabla o hacer un update general sin cambiar solo el estado, se bloquea:
...
CREATE TABLE IF NOT EXISTS "public"."usuarios" (
    ...
    "email_pendiente_verificacion" character varying,
    "token_cambio_email" character varying,
    "token_cambio_email_expira_at" timestamp with time zone
);
...
ALTER TABLE ONLY "public"."usuarios"
    ADD CONSTRAINT "usuarios_token_cambio_email_key" UNIQUE ("token_cambio_email");
