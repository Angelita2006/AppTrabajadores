...
export default function VacacionesScreen() {
  const { usuarioActual, trabajadorActual, empresaActual } = useSesion();
  ...
  const cargarHistoricoAusencias = async () => {
    ...
    try {
      ...
      const ausenciasTrabajador: ItemAusencia[] =
        await obtenerAusenciasTrabajador(trabajadorActual.id);
      ...
      <Card>
        ...
        >
          {cargando ? (
            ...
            <ThemedText style={styles.submitText}>Enviar Solicitud</ThemedText>
          )}
        </Pressable>
      </Card>
